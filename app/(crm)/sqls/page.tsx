...
type Sql = { id?: string; name: string; source: string; course: string; score: number; last: string; assigned: string; status: string; };
...
export default function SQLsPage() {
  const [sqls, setSqls] = useState<Sql[]>(INIT_SQLS);
  const [search, setSearch] = useState("");
  const [toast, setToast] = useState("");
  const showToast = (m: string) => { setToast(m); setTimeout(() => setToast(""), 2500); };

  useEffect(() => {
    fetch("/api/contacts?stage=sql")
      .then((r) => r.json())
      .then((rows: Array<{ id: string; firstName: string; lastName?: string; source?: string; courseInterest?: string; score?: number; owner?: string; updatedAt?: string }>) => {
        if (!Array.isArray(rows) || rows.length === 0) return;
        const mapped: Sql[] = rows.map((r) => ({
          id: r.id,
          ...
          last: "recently",
          assigned: r.owner ?? "",
          status: (r.score ?? 0) >= 80 ? "hot" : (r.score ?? 0) >= 60 ? "active" : "cooling",
        }));
        setSqls(mapped);
      })
      .catch(() => {});
  ...
  async function promoteToOpportunity(sql: Sql) {
    setSqls((prev) =>
      prev.filter((s) => (sql.id ? s.id !== sql.id : s.name !== sql.name || s.source !== sql.source))
    );
    showToast(`✅ ${sql.name} promoted to Opportunity!`);
    if (sql.id) {
      fetch(`/api/contacts/${sql.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ stage: "opportunity" }),
      }).catch(() => {});
    }
  }
          ...
          <tbody>
            {filtered.map((s, i) => (
              <tr key={s.id ?? i}>
                ...
                <td><span style={{ color: statusColor[s.status] ?? "var(--text-2)", fontWeight: 600 }}>{statusLabel[s.status] ?? s.status}</span></td>
                <td>
                  <div style={{ display: "flex", gap: 6 }}>
                    <button className="btn btn-ghost btn-sm" onClick={() => showToast(`📞 Calling ${s.name}...`)}>Call</button>
                    <button className="btn btn-primary btn-sm" onClick={() => promoteToOpportunity(s)}>→ Opp</button>
                  </div>
                </td>
              </tr>
            ))}
